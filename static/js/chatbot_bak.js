document.addEventListener('DOMContentLoaded', () => {
    ...
    //const socket = io('https://home-base.co.kr/chatbot/');  // 로컬 서버에 연결 (배포 시 URL 변경)
    const socket = io('https://home-base.co.kr/chatbot/', { transports: ['websocket', 'polling'], secure: true });
    // 이전 메시지 불러오기
    ...
    document.getElementById('chatbot-send-button').addEventListener('click', () => {
        ...
        if (message !== '') {
            socket.emit('chatbotUserMessage', { userMessage : message, userNickname: userNickname });
